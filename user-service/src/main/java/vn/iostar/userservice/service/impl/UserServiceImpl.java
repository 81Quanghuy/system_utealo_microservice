...
public class UserServiceImpl implements UserService {
	...
	public ResponseEntity<GenericResponse> changePassword(String userId, ChangePasswordRequest request)
		...
		return ResponseEntity.ok(GenericResponse.builder().success(true).message("Đổi mật khẩu thành công")
	...
	public ResponseEntity<Object> updateProfile(String userId, UserUpdateRequest request) throws Exception {
		...
		return ResponseEntity.ok(GenericResponse.builder().success(true).message("ập nhật thông tin người dùng thành công")
	...
	public ResponseEntity<Object> accountManager(String authorizationHeader, UserManagerRequest request) {
		...
		if (!roleName.name().equals("Admin")) {
			return ResponseEntity.status(HttpStatus.NOT_FOUND).body(GenericResponse.builder().success(false)
					.message("Không có quyền truy cập").statusCode(HttpStatus.NOT_FOUND.value()).build());
		}
		Optional<User> user = findById(request.getUserId());
		if (user.isEmpty()) {
			return ResponseEntity.status(HttpStatus.NOT_FOUND).body(GenericResponse.builder().success(false)
					.message("Người dùng không tồn tại").statusCode(HttpStatus.NOT_FOUND.value()).build());
		}
		user.get().getRole().setRoleName(request.getRoleName());
		user.get().getAccount().setIsActive(request.getIsActive());
		save(user.get());
		return ResponseEntity.ok(GenericResponse.builder().success(true).message("Cập nhật tài khoản thành công")
	...
	public ResponseEntity<GenericResponse> getAvatarAndName(String userId) {
		...
		return ResponseEntity.ok(GenericResponse.builder().success(true).message("Lấy thông tin người dùng thành công")
			...
			if (optionalUser.isPresent()) {
				...
				if (updatedUser != null) {
					return ResponseEntity.ok()
							.body(new GenericResponse(true, "Xóa thành công!", updatedUser, HttpStatus.OK.value()));
				}
				/// cập nhật không thành công
				else {
					return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).body(new GenericResponse(false,
							"Cập nhật thất bại!", null, HttpStatus.INTERNAL_SERVER_ERROR.value()));
				}
			}
			/// khi không tìm thấy user với id
			else {
				return ResponseEntity.status(HttpStatus.NOT_FOUND)
						.body(new GenericResponse(false, "Không tìm thấy người dùng", null, HttpStatus.NOT_FOUND.value()));
			}
	...
	public ResponseEntity<GenericResponseAdmin> getAllUsers(String authorizationHeader, int page, int itemsPerPage) {
		...
		if (!roleName.name().equals("Admin")) {
			return ResponseEntity.status(HttpStatus.FORBIDDEN).body(GenericResponseAdmin.builder().success(false)
					.message("Không có quyền truy cập").statusCode(HttpStatus.FORBIDDEN.value()).build());
		}
		...
		if (users.isEmpty()) {
			...
			return ResponseEntity.status(HttpStatus.OK)
					.body(GenericResponseAdmin.builder().success(true).message("Lấy danh sách người dùng thành công")
...
//			groupMemberRepository.saveAll(listMember);
	...
	public ResponseEntity<Object> searchUser(String fields, String query) {
		...
		if (users.isEmpty()) {
			return ResponseEntity.status(HttpStatus.NOT_FOUND).body(GenericResponse.builder().success(false)
					.message("Không tìm thấy thông tin người dùng").statusCode(HttpStatus.NOT_FOUND.value()).build());
		} else {
			return ResponseEntity.status(HttpStatus.OK)
